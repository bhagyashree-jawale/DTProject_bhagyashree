									...
									<tr
										...
										<c:if test="${pageContext.request.userPrincipal.name != null}">
											<c:if
												test="${pageContext.request.userPrincipal.name == 'bhagyashree'}">
												<td width="50"><a
													href="<c:url value="/admin/BulletineEdit?id=${bul.id}"/>">Edit</a></td>
												<td width="50"><a
													href="<c:url value="/admin/BulletineDelete?id=${bul.id}"/>">Delete</a></td>
											</c:if>
										</c:if>
										<td width="50"><a
											href="<c:url value="/BulletineViewRecord?id=${bul.id}"/>">View
												</a></td>
									</tr>
								</c:forEach>
							</table>
